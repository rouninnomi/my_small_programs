...
class Onward_list_generator():
    ...
    def __init__(self, ipt_dir) -> None:
        ...
        # 当直回数のカウンター
        n_days = self.kibou_lists[0].shape[0]
        self.counter_dict = {'勝木': math.floor(n_days/3),
                        '根岸': 0,
                        '大津': 0}
        self.counter_dict['根岸'] = math.ceil((n_days - self.counter_dict['勝木'])*2/3)
        self.counter_dict['大津'] = n_days - self.counter_dict['勝木'] - self.counter_dict['根岸']
...
while olg.result['担当'].value_counts()['いません'] >= 2:
    olg.kibou_ume()
    olg.kyujitsu_ume()
    olg.nokori_ume()
